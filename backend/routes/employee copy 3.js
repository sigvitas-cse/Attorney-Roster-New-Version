...
const US_CITIES = ["new york", "los angeles", "chicago", "houston", "phoenix", "dallas", "san francisco"];
const US_STATES = ["alabama", "california", "texas", "florida", "illinois", "new york", "ohio"];
const COMMON_ORGS = ["inc", "llp", "llc", "law group", "patent", "ip", "firm"];

function isLikelyCity(term) {
  const lower = term.toLowerCase();
  return US_CITIES.some(city => lower.includes(city));
}

function isLikelyState(term) {
  const lower = term.toLowerCase();
  return US_STATES.some(state => lower.includes(state));
}

function isLikelyOrg(term) {
  const lower = term.toLowerCase();
  return COMMON_ORGS.some(keyword => lower.includes(keyword));
}
...
function detectField(term) {
  ...
  if (/^\d{5,6}$/.test(trimmed)) return "regCode";
  if (/^\d{3}[-\s]?\d{3}[-\s]?\d{4}$/.test(trimmed)) return "phoneNumber";
  if (/@|\.(com|org|net|edu)$/.test(trimmed)) return "emailAddress";
  if (/^https?:\/\//.test(trimmed) || /^www\./.test(trimmed)) return "firmUrl";
  if (/^\d+ [A-Za-z0-9]/.test(trimmed)) return "addressLine1";
  if (/^[A-Za-z .\-']{3,}$/.test(trimmed)) {
    if (isLikelyCity(trimmed)) return "city";
    if (isLikelyState(trimmed)) return "state";
    if (isLikelyOrg(trimmed)) return "organization";
    return "name";
  }
...
router.get("/IndivisualDataFetching", async (req, res) => {
  ...
  try {
    const results = await UserModel.find({ $or: conditions }).limit(20); // Limit to 20
    if (!results.length) {
...
router.get("/suggestions", async (req, res) => {
  ...
  const baseFields = ['name', 'organization', 'city', 'addressLine1', 'zipcode'];
  const searchFields = isNumber ? ['regCode', ...baseFields] : baseFields;
  ...
  try {
    const docs = await UserModel.find({ $or: orConditions })
      .limit(20)
      .select([...searchFields, '_id'].join(' ')); // Include only desired fields

    const suggestionSet = new Set();
   
    let detectedField = isNumber ? 'regCode' : '';
    ...
    for (const doc of docs) {
      for (const field of searchFields) {
        if (
          ...
          doc[field].toLowerCase().startsWith(trimmed.toLowerCase())
        ) {
          suggestionSet.add(doc[field]);
          if (!detectedField) detectedField = field; // 🧠 first matching field
        }
      }
    }
    return res.json({
      field: detectedField || (isNumber ? 'regCode' : 'name'),
