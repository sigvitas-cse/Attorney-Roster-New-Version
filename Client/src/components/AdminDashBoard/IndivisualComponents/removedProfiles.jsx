    ...
    const fetchRemovedProfiles = async () => {
      try {
        ...
        if (removedDataResponse.status === 200) {
          setData1(removedDataResponse.data);
        }
      } catch (err) {
  ...
  return (
    <div className="p-2">
      <h2 className="text-[12px] font-semibold text-[#1E293B] mb-2 text-center">
        Removed Profiles [{result.length}]
      </h2>
            ...
            {paginatedData.length > 0 ? (
              paginatedData.map((profile, index) => (
                ...
                >
                  ...
                  <td className="p-1 border-b border-[#CBD5E1] max-w-[100px] truncate">{profile.name}</td>
                  <td className="p-1 border-b border-[#CBD5E1] max-w-[100px] truncate">{profile.organization}</td>
...
export default RemovedProfiles;